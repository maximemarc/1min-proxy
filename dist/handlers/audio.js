...
import { speechRequestSchema, transcriptionRequestSchema, translationRequestSchema, } from '../schemas/index.js';
...
export class AudioHandler {
    ...
    async transcription(req, res) {
        try {
            ...
            const data = await this.api.features.speechToText(mappedModel, String(audioUrl), {
                language,
            });
    ...
    extractAudioUrl(data) {
        ...
        if (aiRecord?.aiRecordDetail) {
            ...
            const url = Array.isArray(result) ? result[0] : String(result);
